...
SOURCE = 'funksjon start() -> ordbok_tekst { returner {"en": "1", "to": "2"} }\n'


def _run_selfhost_bytecode() -> dict[str, object]:
    completed = subprocess.run(
        ["./bin/nc", "run", "selfhost/tests/map_parity.no"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def _run_python_bytecode() -> dict[str, object]:
    with tempfile.TemporaryDirectory() as tmpdir:
        source_path = Path(tmpdir) / "map.no"
        source_path.write_text(SOURCE, encoding="utf-8")
        result = compile_source(str(source_path))
        return result.bytecode


def test_selfhost_map_literal_matches_python_contract() -> None:
