...
def _run_selfhost_bytecode() -> dict[str, object]:
    completed = subprocess.run(
        ["./bin/nc", "run", "selfhost/tests/match_parity.no"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def _run_python_bytecode() -> dict[str, object]:
    with tempfile.TemporaryDirectory() as tmpdir:
        source_path = Path(tmpdir) / "match.no"
...
def _normalize_code(code: list[list[object]]) -> list[list[object]]:
    ...
    def normalize_name(value: str) -> str:
        nonlocal next_index
        if value.startswith(("__L", "match_subject_", "match_next_", "match_end_", "_match_subject_", "__match_subject_")):
    ...
    for instr in code:
        ...
        if op in {"TRY_BEGIN", "JUMP", "JUMP_IF_FALSE", "LABEL"} and len(instr) > 1 and isinstance(instr[1], str):
            ...
            if len(rewritten) > 1 and isinstance(rewritten[1], str):
                rewritten[1] = normalize_name(rewritten[1])
...
def test_selfhost_match_matches_python_contract() -> None:
