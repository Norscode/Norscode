...
SOURCE = "funksjon start() -> heltall { returner 1 + 2 }\n"


def _run_selfhost_bytecode() -> dict[str, object]:
    completed = subprocess.run(
        ["./bin/nc", "run", "selfhost/tests/bytecode_parity.no"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def _run_python_bytecode() -> dict[str, object]:
    with tempfile.TemporaryDirectory() as tmpdir:
        source_path = Path(tmpdir) / "parity.no"
        source_path.write_text(SOURCE, encoding="utf-8")
        result = compile_source(str(source_path))
        return result.bytecode


def test_selfhost_bytecode_matches_python_contract() -> None:
