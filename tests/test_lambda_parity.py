...
SOURCE = 'funksjon start() -> heltall { la x = fun(a) -> a + 1 returner 1 }\n'


def _run_selfhost_bytecode() -> dict[str, object]:
    completed = subprocess.run(
        ["./bin/nc", "run", "selfhost/tests/lambda_parity.no"],
        text=True,
        capture_output=True,
        check=False,
    )
...
def _run_python_bytecode() -> dict[str, object]:
    with tempfile.TemporaryDirectory() as tmpdir:
        source_path = Path(tmpdir) / "lambda.no"
        source_path.write_text(SOURCE, encoding="utf-8")
        result = compile_source(str(source_path))
        return result.bytecode


def test_selfhost_lambda_matches_python_contract() -> None:
