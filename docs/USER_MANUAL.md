...
- `std.admin`: CRUD-admin for registrerte modellar, med escaped HTML-output, strenge no-store/CSP/HSTS admin-headers på sider og redirects, ingen inline admin-JavaScript, oversiktsstatistikk, tom-tilstand, kontrollerte statusmeldingar, audit-eventar og CSRF-sikre admin-POST-hjelparar.
