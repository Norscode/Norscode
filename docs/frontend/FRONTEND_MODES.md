...
Norsk stilark mode er den norske CSS-varianten i Norscode.

Bruk når:

- du vil bygge stilark med norske funksjonsnamn
- du vil halde stilreglar i Norscode-kode i staden for rein CSS-tekst
- du vil generere variantar av same tema frå kode
...
Når du byggjer eit fullstendig tema, kan du kombinere `tema_lys(...)`, `tema_mørk(...)`, `token_farge(...)`, `token_spacing(...)` og `token_ref(...)`.

For meir avanserte variantar kan du òg bruke `property_regel(...)`, `scope(...)` og `url(...)` når du må nærme deg moderne CSS utan å gå tilbake til rå tekst.
