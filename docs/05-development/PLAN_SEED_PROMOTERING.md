...
| **`\uXXXX` gjev ugyldig UTF-8 på fersk seed** | `[ ]` | Dette avviket er derimot ekte, og no målt på teiknkodar i staden for stringify. `json_parse("\"\\u00e6\"")` → committed: len=6, koder `[92 117 48 48 101 54]` (escapen blir ikkje dekoda i det heile); fersk: len=1, kode `[230]` — dekoda til **Latin-1**, ikkje UTF-8. Rett UTF-8 for æ er `[195 166]`. Fersk seed produserer altså ugyldig UTF-8 ut av gyldig JSON. Rå «æ» i kjelda er lik på begge (`[195 166]`). Same familie som minnet `native-x86-seed-json-unicode-bug`. |
