...
5. `[~]` Slett C-arkivet + `build/v3009/*.c` (fjern ~20 assert-linjer i preflight/ownership), `archive/legacy_shell/` (119 .sh), `Dockerfile` (broten python:3.12-image) + `Dockerfile.linux-build`. **DELVIS (grein `fase-c5-slett-c`):** shell-arkivet (118 av 120), `Dockerfile`, `Dockerfile.linux-build`, `.dockerignore` sletta; 217 `archive/legacy_shell`-assertar fjerna frå preflight; `tools/gc_probe_run.no` erstattar `gc_probe_run.sh` som utviklarløkke. **BLOKKA:** (a) `archive/legacy_shell/tools/{gc_probe_run,gc_litmus_run}.sh` må stå til `.github/workflows/gc-litmus.yml` (~42 `sh archive/legacy_shell/...`-steg) er bytt til `./bin/nc run tools/gc_probe_run.no …`; (b) C-arkivet + `build/v3009/*.c` er LEVANDE CI: `ci.yml` job `linux-arm64-runtime-attestation` → `tools/build_linux_arm64_tls_attestation_candidate.no:118` køyrer `gcc -Iarchive/legacy_c_backend build/v3009/native_candidate_gc.c` (grøn på main). C-en kan fyrst gå når den jobben er bytt til kjelde-codegen (`arm64-native-attest.yml`-mønsteret).
