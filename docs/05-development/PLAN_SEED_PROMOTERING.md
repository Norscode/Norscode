...
| ELF stage-0 fixpunkt (Gen1 == Gen2) | `[~]` | A3-fiksen krev regenererte fragment; regenerert i Docker, paritetskøyring pågår |
...
- **A3 Kompilator: builtin skal vinne over ukvalifisert import** `[x]` kode / `[~]` fragment — `legg_til(l, x)` i `__main__` vart `CALL std.dns.legg_til` når `std.dns` var importert (`imported_funk_kart`). Fiks i `ir_to_bytecode.registrer_importerte_funksjonar` + utvida `semantic.er_builtin`. Verifisert på seed AE: `test_dns_ds_record` OK. Fragment regenerert (Gen2 frå Docker), paritetskøyring pågår.
- **A4 Push + ny CI-runde** `[ ]` — 14 lokale commits + dagens (A1–A3, plan) skal pushast når fragment-pariteten er stadfesta lokalt. Deretter `gh workflow run b2-seed-direct.yml` for seed-port-tabellen.
...
| Jobb | Kvar | Forventa |
|---|---|---|
| Fixpunkt-paritet Gen1 == Gen2 med nye fragment | Docker linux/amd64 | ~20 min |
| Deretter: commit A3 + fragment, push, CI-runde + B2 fullhost | GitHub | CI ~3–4 t |
| Parallelt: profiler `test_security` på seed AE | Docker | — |
