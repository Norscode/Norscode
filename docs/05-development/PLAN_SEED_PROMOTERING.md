...
Levande statusplan. Oppdatert av kvar commit som endrar status. Sist oppdatert: **2026-09-07 (kveld, 21:45)**.
...
| B2 «Fullhost nc_main native seed» | `[~]` ROT-ÅRSAK: json_stringify | Materialize-korrupsjonen var `builtin.json_stringify` (legacy-serializer) på Linux-stage0, ikkje GC (sjå Fase B). Fiks: `json_skriv`. Fullhost-verifisering (tiny42 på rein kandidat) pågår. Jobben er promoterings-diagnose → ikkje PR-blokkerande |
