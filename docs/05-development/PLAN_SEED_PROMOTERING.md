...
Levande statusplan. Oppdatert av kvar commit som endrar status. Sist oppdatert: **2026-09-10 — desimaltall nativt (PR #194), bygg-native-null-fallback funnen**.
...
| **Native `desimaltall` (flyttal)** | `[x]` **FIKSA — PR #194** (var BLOKKAR PROMOTERING) | Var klassifisert «utanfor porten» som ein manglande funksjon. Målingar 2026-09-10 oppgraderer den til blokkar: det er ikkje fråvær, det er **stille miskompilering**. Docker linux/amd64, same fil, same køyring, committed vs fersk seed: `3.5`→**3**, `0.1`→**0**, `3.5 + 2.5`→**5**, `json_parse_raw("3.5")`→`heltall 3`, `json_parse_raw({"n":{"m":2.5}})`→`{"m":2}`, `builtin.desimaltall("3.5")`→**SIGSEGV (rc=139)**. Ingen test fanga det (strukturelle testar les kjeldetekst; funksjonelle rekna berre heiltal) → `tests/test_desimaltall_runtime.no` er lagt til som vakt, verifisert grøn på committed og raud på fersk. **ROT (presisert):** dette er ikkje ein parserfeil. Den **native x86-64-backenden har ingen `desimaltall`-NcVal-type i det heile** — `native_codegen_v2.no` viser `builtin.desimaltall` rett til heiltalsrutina (`la er_to_int = fn_nm == "builtin.desimaltall"` :6177 → `hvis er_to_int { returner RT_TO_INT() }` :6248). `json_parse` trunkerer fordi det ikkje finst noko flyttal å produsere: ingen `RT_FLOAT`, ingen xmm-boksing, ingen flyttalsaritmetikk. VM-en/C-runtimen HAR typen (`vm.no:735`), so committed seed er frisk; det er den self-hosta backenden som manglar heile typen. C.3 er forkravet for å kunne redigere rutinene, men lukkar det ikkje — **verifisert 2026-09-10: C.3-seed byggjer reint (rc=0, 4 230 039 B) og feilar framleis vakta med same melding**, fordi C.3 med vilje er ein byte-identisk transkripsjon av blobben. Snarvegen «skriv desimalar som strengar i NCB» hjelper heller ikkje, sidan det ikkje finst ein type å konvertere til. Estimatet «2–3 veker» i den gamle raden ser rett ut. |
| **`builtin.json_parse`-gapet: 136 000× tregare** | `[ ]` | Målt på fersk seed, same 704 KB NCB, same køyring: `json_parse_raw` (native RT) **3 ms** — altså raskare enn committed seed (22 ms) — mot `json_parse` (rutar til `std.native_gap.json_parse`) **409 292 ms**. Gap-en er teikn-for-teikn med `builtin.slice(s,p,p+1)`, éin strengallokering per teikn, og gjer den levande mengda så stor at GC-markinga multipliserer kostnaden på toppen. Med 379 kallstader i repoet er dette ikkje eit hjørne. **FELLE:** den opplagde fiksen — `json_parse_raw` og så serialiser nodane tilbake til tekst — ville REGREDERE flyttal. Den noverande teikn-parsaren les talet som TEKST rett frå kjelda og er difor immun mot trunkeringa; ein raw-parse-og-reserialiser ville gjere `json_parse("3.5")` til `"3"` på alle 379 kallstadene. Ein slik fiks må anten hente det opphavlege tekstspennet for tal (som `json_parse_raw` ikkje gjev), eller vente på at desimaltal-typen finst. Alternativet som IKKJE har den fella: behald teikn-parsaren, men fjern allokeringa per teikn (`builtin.slice(s,p,p+1)` lagar éin streng per teikn) — då er fiksen mogleg utan C.3, sidan `std/native_gap.no` er rein Norscode. **Er 3 ms mot 409 s ei rettvis samanlikning?** Ja, og det er verdt å skrive ned sidan kvar køyring viser `[vm] entry returned` og det ser ut som tolking. Kjeda er: brukarprogrammet blir tolka, VM-en dispatchar `builtin.json_parse` (`vm.no:2980`) til vertsruntimen sin, og på fersk seed er VM-EN SJØLV kompilert av `native_codegen_v2` — der går det kallet til gap-ruta (`:7025–7028`). Sjølve parse-lykkja køyrer altså som NATIVE KOMPILERT kode; berre det ytste kallet er tolka. På committed seed går same dispatch til C-builtinen. Native mot native. |
| ~~«gap-en er ikkje kontrakt-tru»~~ | **RETRAKTERT** | Eg påstod fyrst at gap-en gav nøsta verdiar som JSON-strengar der committed gav typa verdiar. Det var FEIL, og feilkjelda er verdt å hugse: eg samanlikna via `json_stringify`, og den gamle seeden sin `json_stringify` er sjølv broten (legacy-serializer skriv streng → bar token, jf. c161675). Samanlikna på TYPE og LENGD i staden er dei **byte-identiske** på begge seedar: objekt→ordbok, liste→ordbok med indeksnøklar, alle medlemer→tekst (`o[e]` = `<[1,2]>`, `n[n]` = `<{"m":2}>`). Legacy-kontrakten er altså oppfylt, og dei 379 kallstadene reparsar nøsta verdiar med vilje (t.d. `acme_pebble_issuance_gate.no:101`). |
| **`\uXXXX` gjev ugyldig UTF-8 på fersk seed** | `[x]` fiksa — PR #194 | Dette avviket er derimot ekte, og no målt på teiknkodar i staden for stringify. `json_parse("\"\\u00e6\"")` → committed: len=6, koder `[92 117 48 48 101 54]` (escapen blir ikkje dekoda i det heile); fersk: len=1, kode `[230]` — dekoda til **Latin-1**, ikkje UTF-8. Rett UTF-8 for æ er `[195 166]`. Fersk seed produserer altså ugyldig UTF-8 ut av gyldig JSON. Rå «æ» i kjelda er lik på begge (`[195 166]`). Same familie som minnet `native-x86-seed-json-unicode-bug`. |
| **`bygg-native` på fersk seed skriv INGEN ELF** | `[ ]` eiga grein `host-exec-ncb-json` | Gammal feil, ikkje laga av flyttalsarbeidet: stadfesta på ein seed bygd frå #193-tuppen utan desse endringane (rc=0, ingen ELF, 0 B ut). `host_exec_ncb_json` var ein C-æra-vertsbuiltin; kodegeneratoren har inga rute for han, so kallet i kompilert `nc_main.no` (bygg-native → `nc_bygg_native_køyr_codegen`) blir null-fallback og returnerer 0 — `bygg-native` rapporterer suksess utan å ha gjort noko. Seed-bygglogen har sagt `ÅTVARING ukjent kall builtin.host_exec_ncb_json/1 → null-fallback` heile tida. Tolka kallarar (t.d. `ncb_host_run_entry.no` via `run`) treffer VM-dispatchen i `vm.no` og verkar — det er berre kompilert kode som er råka. |
...
- `[x]` `desimaltall` nativt — PR #194 (VM, AOT, literalar, tekst, samanlikning)
...
- 2026-09-10 kveld: **desimaltall nativt** (PR #194, base #193). Rota var tre lag: `builtin.desimaltall`→`RT_TO_INT`, `builtin_va` før `gap_rute_v2`, og json_parse-atomet som kasta brøken. Maskinkode for type/rekning, Norscode (`std/desimaltall_tekst.no`) for tekst↔double. `test_desimaltall_runtime` raud→grøn; tre testar lagde til seed-porten (111). #191 og #193 grøne. Funne: `bygg-native` null-fallback (sjå Fase B).
