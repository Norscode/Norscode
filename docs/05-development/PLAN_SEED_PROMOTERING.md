...
| Binær NCB-kodar bulk (Fase C.4) | `[x]` KODAR + LESAR-side FERDIG | Den gamle 8,1 GB-OOM-en er borte: `selfhost/ncb_bin.no` brukar no varint/zigzag + bulk-append og strippar debug-metadata (`source_lines`/`source_columns`). **VERIFISERT:** `NORSCODE_NCB_BINARY=1` kompilerer `selfhost/vm.no` (1,11 MB JSON) → 785 KB binær (−29 %), 0 OOM, magic `93 4E 43 42 01` korrekt; committed seed les+køyrer binær NCB via `run-ncb-pure` (round-trip OK). **Lesar-side (ikkje-fikspunkt-modular) lukka i kjelde:** `l5b_host.no`, `l5b_vm_runner.no`, `ncb_pure_run_entry.no` går no via dual-format `serde` (JSON-åtferd byte-identisk, binær auto-detektert på magic). **`bundler.no` er UNNTATT (utsett til reseed):** han er ein av dei 8 kompilator-bunt-modulane i `selfcompile-stage0-elf` sitt Gen1→Gen2 byte-paritet-fikspunkt, og `serde.les_fil` (rein Norscode-funksjon) fell til null-fallback i den frittståande kompilator-bunten (utan ncb_serde), medan `json_parse_raw`/`fil_les` er native atomar → fikspunkt-brest (Gen1 1206469 B != Gen2 1206351 B). Bundler-lesarane må difor migrerast saman med reseed-en, når ncb_serde blir del av bootstrap-bunten og fragmenta regenererast. Att for full aktivering: flipp on-disk-format (skrivar-flagg + konvertering av `.ncb.json`) — gata på reseed (sjå RESEED_BLOKKAR_ANALYSE.md) så seeden sine innebygde lesarar òg er dual-format. |
...
**Inventar 2026-09-07 (utforskingsagent, sjå minne `fase-c-inventar`):** CI er ALT fri for gcc/clang/python/pip/jq/node. 0 Python-filer. 13 C/H-filer (archive/legacy_c_backend + build/v3009) haldne i live berre av innhaldsassertar i `tools/release_preflight.no` + `verify_norscode_surface_ownership.no`. 212 JSON-filer (11,6 MB), 92 `.ncb.json` (9,3 MB). Frosen maskinkode i `native_codegen_v2.no`: 10 913 B (`rt_hex_del0–6` + 2 inline-halar) + 1 999 B `rt_hex_process_spawn` = 12,6 KiB; 70 `RT_*`-adressekonstantar: 9 trampolinerte, 20 daude, **39 framleis levande** (~9 KiB C-æra-kode). *(Utdatert: sjå pkt. 3 — blobben er borte sidan `518699e`.)*
...
3. `[x]` **GJORT (korrigert 2026-09-25):** dei 39 rutinene vart Norscode-atom i `8f1050a`, og blobben `rt_hex_del0–6` (10 913 B), dei påhengde halane og replace-patchane vart sletta i `518699e` (C.3, 2026-09-09). Alle 50 `RT_*`-slottar peikar no på Norscode-emitterte atom (`native_codegen_v2.no:115–173`). Einaste frosne maskinkode att er `rt_hex_process_spawn_linux_x86_64` (1 999 B, `native_codegen_v2.no:2021`), som får eigen byte-identisk omskriving. Opphavleg punkt: **Emitter dei 39 levande frosne rutinene som Norscode-atomics** (mønster: 90 atomics + `patch_abs_jump` finst alt). Tyngdepunkt: `RT_JSON_PARSE` (~1,1 KB), `RT_SPLIT`/`RT_REPLACE`/`RT_STR_TO_INT`, `RT_LIST_*`, `RT_MAP_KEYS`/`RT_MAP_VALS` (mest presserande: legacy-lesarar på ny map-layout), `RT_INDEX_GET/SET`, `RT_BUILD_{LIST,MAP}_REV`, `RT_FIL_LES`/`RT_FIL_SKRIV*`, `RT_MILJO_HENT`, 12 aritmetikk/samanlikning, `RT_CONCAT`/`RT_INT_TO_STR`, `RT_INIT_HEAP`, `process_spawn`-blobben (eige atomic). **Blobben må vekk i EITT jafs** (alle RT_* er absolutte VA-ar) → så slett `rt_hex_del*`, `hex_to_bytes`, dei ~25 `replace`-patchane og `patch_*`-funksjonane.
4. `[~]` NCB JSON → binær (`.ncbin`) overalt: (a) `[x]` ncb_bin bulk-kodar (varint/zigzag + metadata-strip, verifisert, sjå rad over); (b) `[x]` skrivaren finst og er verifisert (`NORSCODE_NCB_BINARY=1` i `nc_main` compile + bundle); (c) `[~]` LESAR-side lukka i kjelde (alle NCB-lesarar via `serde`, dual-format) — sjølve konverteringa/regenereringa av dei 78 `.ncb.json` er gata på reseed (den committa seeden sine INNEBYGDE lesarar må òg vere dual-format før disk-formatet kan snuast utan å knuse CI); (d) `elf_compile_driver.no` les 8 `precompiled_fragments_inner/*.functions.json` hardkoda — den EINASTE harde JSON-avhengnaden i bootstrap; fersk seed treng dei ikkje. `nc_main` sine `stdlib`/`precompiled`-oppslag er mjuke (fallback til kjelde; risiko = AOT-heap, som GC-modus løyser).
5. `[~]` Slett C-arkivet + `build/v3009/*.c` (fjern ~20 assert-linjer i preflight/ownership), `archive/legacy_shell/` (119 .sh), `Dockerfile` (broten python:3.12-image) + `Dockerfile.linux-build`. **DELVIS (grein `fase-c5-slett-c-2`):** shell-arkivet (118 av 120), `Dockerfile`, `Dockerfile.linux-build`, `.dockerignore` sletta; 217 `archive/legacy_shell`-assertar + 18 `krev_rel_innhald(root, "archive/legacy_c_backend/…")`-assertar fjerna frå preflight (preflight held ikkje lenger C-arkivet i live); `tools/gc_probe_run.no` erstattar `gc_probe_run.sh` som utviklarløkke (verifisert på committed macOS-seed: `NORSCODE_GC_PROBE=tests/fixtures/gc_f1_probe.no ./bin/nc run tools/gc_probe_run.no` → NCB + 42 383-byte ELF, 93 s; posisjonsargument er usynlege på committed seed — bruk miljøforma). **BLOKKA (krev endring i `.github/workflows/**`):** (a) `archive/legacy_shell/tools/{gc_probe_run,gc_litmus_run}.sh` må stå til `gc-litmus.yml` sine 61 `sh archive/legacy_shell/tools/gc_probe_run.sh <probe> <LABEL>`-steg er bytte til `NORSCODE_GC_PROBE=<probe> NORSCODE_GC_PROBE_LABEL=<LABEL> ./bin/nc run tools/gc_probe_run.no` og dei 2 `gc_litmus_run.sh`-stega til `NORSCODE_GC_PROBE=tests/fixtures/gc_litmus_hmac.no NORSCODE_GC_PROBE_LABEL=LITMUS ./bin/nc run tools/gc_probe_run.no`; (b) C-arkivet + `build/v3009/*.c` er LEVANDE CI: `ci.yml` job `linux-arm64-runtime-attestation` → `tools/build_linux_arm64_tls_attestation_candidate.no:118` køyrer `gcc -Iarchive/legacy_c_backend build/v3009/native_candidate_gc.c`. Når C-en går, må òg desse ut/justerast: 9 testar som les C-kjelda (`test_arm64_direct_candidate_dispatch`, `test_contains_any_runtime`, `test_dynamic_sqlite_adapter_contract`, `test_module_globals_ncb_contract`, `test_native_filesystem_symlink_contract`, `test_native_tool_cache_contract`, `test_ncb_to_c_builtin_signature_contract`, `test_platform_abi_source`, `test_vm_vent_sleep`), `tools/windows_runtime_cross_compile_gate.no`, `tools/maint/{refresh_embedded_runtime,splice_native_argv_dispatch,compose_generated_c_batches,inject_vm_lex_c,splice_generated_function(s)}.no` (+ `test_splice_native_argv_dispatch_contract`, preflight `krev_rel_fil` på splice/test_platform_abi_source), `verify_norscode_surface_ownership.no` sitt `build/v3009`-unntak (l. 62, 387–397, 447–449). **F0 (2026-09-25, grein `f0-rydding`):** `archive/` er tømd (berre README med peikar til git-historikken), og `tools/maint/*`, `tools/generate_build_embed_c.no`, kontrakttestane deira og preflight-/ownership-linjene er sletta.
