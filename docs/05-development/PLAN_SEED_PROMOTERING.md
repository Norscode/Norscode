...
Levande statusplan. Oppdatert av kvar commit som endrar status. Sist oppdatert: **2026-09-08 (natt, 2)**.
...
| **random_hex-kollisjon (fersk seed)** | `[~]` ROT-ÅRSAK, fiks spesifisert | Fyrste reelle fersk-seed KØYRETIDS-bug (via seed-porten): `test_auth_mfa_enrollment_recovery` assert #10. Gap-ruta `std.sha256.random_hex_pure` seedar berre frå `tid_ms()` per kall → kall i same ms gjev IDENTISK output (6/64 unike vs committed 64/64) → duplikate MFA-recovery-kodar → eingongsbruk brote. Råkar ALL tryggleiks-random på fersk seed. FIKS: getrandom(2)-syscall-atomic i native_codegen_v2 (fjern gap-ruta linje 6078); interim: tid_ms+heap_bump/persistent teljar i native_gap. Krev seed-rebygg. Sjå minne `fersk-seed-random-hex-kollisjon`. |
