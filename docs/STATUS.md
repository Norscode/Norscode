...
- NCB→ELF-releasebanen er no native-only: `tools/ncb_to_elf.no` brukar
  ...
  vedlikehaldsreserve.
- Windows x86_64 runtime, IOCP/SChannel, AppContainer, filesystem, prosess og
  Argon2id er køyrde på ekte Windows og signerte med GitHub-provenance.
  Cross-gaten legg compiler/VM/executor-NCB inn i PE-fila og verifiserer PE32+
  direkte i Norscode utan avhengigheit til `file`. Samla CI-køyring
  `30788036517` batt Windows-, macOS- og begge Linux-rapportane til commit
  `39f0447dcabb22d54ecaa331b933e6593ab438ca` og gav
  `production_ready_windows=true` og `production_ready_all_platforms=true` for
  akkurat den historiske kandidatgenerasjonen. Gjeldande ikkje-promoterte
  fixed14-kandidat er berre komplett attestert lokalt på macOS og skal
  rapporterast `production_ready_all_platforms=false` fram til nye signerte
  rapportar frå macOS, Linux x86-64, Linux ARM64 og ekte Windows bind same
  kjeldegenerasjon.
- Tracked `bootstrap/stage0/norscode-windows-x86_64.exe` er kontrollert
  promotert etter ekte Windows-attestasjon. Stage0 og fersk cross-kandidat er
  byte-identiske med SHA-256
  `534917fff7781c1c55c4aca322edb5f30295618683766a771205470ea20c00b0`;
  førre attesterte generasjon er bevart som verifisert rollback-artefakt.
...
- Kryptografiflatene brukar reine Norscode-implementasjonar av SHA-256/HMAC, scrypt, BLAKE2b og Argon2id. `std.scrypt` har RFC 7914-vektor og harde pure-VM-parametergrenser; `std.argon2id` har RFC 9106-vektor og OpenSSL/Zig er berre eksplisitte differential-/plattformadapterar.
...
- `std/argon2id_pure.no`, `std/blake2b.no` og `tests/test_argon2id_rfc9106_pure.no` verifiserer prehash og endeleg RFC 9106-tag utan eksternt kryptobibliotek. `std.argon2id` brukar denne banen som standard med konstanttidskontroll; `hash_native_adapter` er valfri og blir differentialtesta separat.
- `std.tls_acme` har no full RFC 8555-flyt med HTTPS directory og Replay-Nonce, ny konto med `jwk`, vidare kall med `kid`, ordre, autorisasjon, challenge, CSR-finalisering og sertifikatnedlasting. Linux CI brukar ein fastlåst Pebble-digest, vel `dns-01`, publiserer korrekt JWK-bunde TXT-svar og lar Pebble validere mot Norscode sin eigen autoritative DNS-daemon over RFC-frama DNS/TCP. `PEBBLE_VA_ALWAYS_VALID` og den eksterne challenge-serveren er fjerna. Ekte Windows har no signert ACME-signering/verifisering; rein Norscode TLS/krypto og ekstern sikkerheitsrevisjon står att før modulen kan løftast frå eksperimentell til stabil.
- `std.sikkerheit` samlar no passordkrypto og capability-policy gjennom ein eksplisitt Norscode-native fasade; algoritmegrenser, pure-VM fallback, deny-by-default og signert ekte køyring på macOS, Linux x86_64/ARM64 og Windows er testa, medan rein Norscode TLS/krypto og ekstern sikkerheitsrevisjon framleis står att.
...
- Aktiv macOS ARM64-runtime har native PBKDF2-HMAC-SHA256 via CommonCrypto og valfri Argon2id-adapter via OpenSSL; standard Argon2id og scrypt er reine Norscode-implementasjonar. Linux ARM64 TLS-attestasjon har framleis ei GCC/C/OpenSSL-overgangsbane, men dei aktive Zig-byggverktøya er fjerna. Kjende PBKDF2-, scrypt-, BLAKE2b- og Argon2id-vektorar er grøne; nye plattformattestasjonar må bindast til gjeldande kandidatgenerasjon.

- Lokal macOS ARM64-runtime vart tidlegare promotert med OpenSSL 3.6.2 for den valfrie Argon2id-adapteren og ACME/DNSSEC-signering. Committed macOS ARM64- og Linux x86_64/ARM64-stage0 er hashfesta i `bootstrap/stage0/SHA256SUMS`; dei blir ikkje endra før den nye kandidaten består full promoteringsport.
...
- Windows x86_64-kandidaten blir no materialisert direkte frå committed PE-stage0 og Norscode-eigde NCB-bilete gjennom `tools/build_windows_stage0_candidate.no`. To uavhengige kandidatbygg må vere byte-identiske; dette er ikkje den separate Gen1→Gen2-selfhost-porten. Aktiv Windows-bygging i CI/release brukar ikkje Zig eller C-bundlarar.
- Windows-kandidatgaten køyrer i hovud-CI med eksplisitt `process.exec`/disk-scope og lastar opp den sjølvstendige kandidat-`.exe`-fila som diagnoseartefakt. Kandidaten må framleis vere identisk med committed stage0 før kontrollert promotering.
- `tests/test_windows_process_appcontainer.no` attesterer prosess-ABI, Job Object, AppContainer/no-network og timeout direkte i Norscode på faktisk Windows-host.
- Windows-release-workflowen køyrer Norscode-eigd kandidatbygging, AppContainer/Job Object, SChannel, native network, IOCP og filesystem-testar før app-pakking.
...
- Plattformkorrigering 2026-08-03 for runtime-avsnittet over: Windows HANDLE-filesystem, CreateProcess/Job Object og SChannel er no køyrde på ekte Windows; macOS CRL/OCSP/mTLS og Linux x86-64/ARM64 TLS er signert attesterte. Dei opne runtimepunkta er rein Norscode TLS/krypto, Windows-thread/freestanding atomics og dei eksplisitte fase-5-overgangane, ikkje lenger manglande ekte plattformkøyring.
- Pure-VM-sikkerheit er no handheva før builtin-dispatch: fil, mappe, lagring, database, miljø, prosess og socket/nettverk krev eksplisitte capabilities. Standard er deny, native filhandtak avviser traversal og symlink-folging, read/write-builtins kan ikkje kryssbrukast, og auditmåling viser tillatne og avviste kall. Nye listen/connect/TLS-handtak må samsvare med network scope, og executable må samsvare med process scope. Prosess-ABI-et avviser no ukjende sandboxprofilar, ugyldige miljøflagg og minne-/fd-grenser før dispatch. Prosessar kan bruke lukka miljø, rlimits, Linux seccomp no-network/no-write/pure og macOS Seatbelt med RLIMIT_RSS/RLIMIT_DATA og forelder-watchdog; Windows AppContainer og Job Object er no køyrde og signert attesterte på ekte Windows.
...
- `std.prosess` har levande POSIX-prosesshandtak med shell-fri `fork`/`execv`, PID, ikkje-blokkerande stdin/stdout/stderr, poll, timeout/deadline, TERM/INT/KILL/HUP, exit-kode, inkrementell streaming og slot-gjenbruk. Prosesshandtak kan fullføre scheduler-futures som vekkjer suspendert VM-`await`. Sandboxprofil gir lukka miljø, rlimits, Linux seccomp no-network/no-write/pure og macOS Seatbelt no-network/no-write med Darwin RSS/DATA-grense og forelder-watchdog. Windows CreateProcess og Job Object er køyrde og signert attesterte på ekte Windows.
