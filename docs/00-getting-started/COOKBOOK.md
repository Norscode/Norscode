...
Se også [examples/cli.no](../examples/cli.no).
...
Se også [examples/cli.no](../examples/cli.no) og [examples/helpdesk.no](../examples/helpdesk.no).
...
Se [examples/http.no](../examples/http.no) for en nettfri demo av `std.http`-overflata.
...
Se [examples/web.no](../examples/web.no) og [examples/web_request_response.no](../examples/web_request_response.no).

For route-handlers i FastAPI-stil, se også [examples/web_routes.no](../examples/web_routes.no).
For subroutere og prefiksbaserte route-moduler, se [examples/web_subrouter.no](../examples/web_subrouter.no).
For route-guards og policy-hooks, se [examples/web_guard.no](../examples/web_guard.no).
For HEAD, OPTIONS og metodeforhandling, se [examples/web_methods.no](../examples/web_methods.no).
For bearer-token auth med guards, se [examples/web_auth.no](../examples/web_auth.no).
For rolle- og rettighetsmodell, se [examples/web_roles.no](../examples/web_roles.no).
For passordhashing og secrets-håndtering, se [examples/secrets.no](../examples/secrets.no).
For CSRF-beskyttelse, se [examples/csrf.no](../examples/csrf.no).

For secure cookies, se [examples/web_cookies.no](../examples/web_cookies.no).
For input-sanitizing og sikre web-strenger, se [examples/web_sanitize.no](../examples/web_sanitize.no).
For databasebruk med SQLite og migreringer, se [examples/db.no](../examples/db.no).
For ekte databaseintegrasjon med reopen og persistens, se [examples/db_integration.no](../examples/db_integration.no).
For repository-/modelmønstre, se [examples/db_repository.no](../examples/db_repository.no).
For enkel JSON-/schema-mapping, se [examples/json_schema.no](../examples/json_schema.no).
For fil- og objektlagring, se [examples/file_object_storage.no](../examples/file_object_storage.no).
...
funksjon start() -> heltall {
    ...
    skriv(cache.hent(c, "bruker:99", "ukjent"))
    returner 0
}
```

Se [examples/cache.no](../examples/cache.no).
...
Se [examples/logging.no](../examples/logging.no).
...
Se [examples/metrics.no](../examples/metrics.no).
...
Se [examples/trace.no](../examples/trace.no).
...
Se [examples/audit.no](../examples/audit.no).
...
Se [examples/observability_export.no](../examples/observability_export.no).

For dependency injection i samme stil, se [examples/web_dependency.no](../examples/web_dependency.no).

For inputvalidering, se [examples/web_validation.no](../examples/web_validation.no).

For OpenAPI og docs, se [examples/web_openapi.no](../examples/web_openapi.no).

For OpenAPI med bearer-auth, se [examples/web_openapi_auth.no](../examples/web_openapi_auth.no).

For OpenAPI med dokumenterte JSON-feilresponser, se [examples/web_openapi_errors.no](../examples/web_openapi_errors.no).

For OpenAPI med nestede objekt-skjemaer, se [examples/web_openapi_schema.no](../examples/web_openapi_schema.no).

For API-versjonering og migrasjonsnotater, se [docs/API_VERSIONING.md](./API_VERSIONING.md) og [docs/API_MIGRATION_NOTES.md](./API_MIGRATION_NOTES.md).
...
For en samlet deployflyt fra release til drift, se [docs/DEPLOYMENT_PLAYBOOK.md](./DEPLOYMENT_PLAYBOOK.md).
For å starte et nytt API-prosjekt, se [docs/API_SCAFFOLD.md](./API_SCAFFOLD.md).
