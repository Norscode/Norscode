...
- generell Linux-installasjon finst i `tools/install.sh`
...
Status:
Bygd i første praktiske form. `tools/package-linux-app.sh` lagar no eit `AppDir`, pakkar det som tarball, og prøver `AppImage` som beste innsats naar køyringa skjer paa Linux med `appimagetool`.
...
Status:
Bygd i første praktiske form. `tools/install-linux-desktop-entry.sh` installerer no `.desktop`-entry og ikon fraa `AppDir` til brukarens lokale desktop-flate, og flyten er dokumentert i `docs/LINUX_APP_DESKTOP.md`.
...
Status:
Bygd i første praktiske form. `tools/install-linux-appdir.sh` installerer no AppDir-tarballen repeterbart med versjonert layout og aktiv `current`-lenke, og brukarinstallasjonen er dokumentert i `docs/LINUX_APP_INSTALL.md`.
