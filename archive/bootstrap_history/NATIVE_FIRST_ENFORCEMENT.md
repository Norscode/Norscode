...
- **Maintainer verification:** `tools/selfhost_maintenance_verify.sh` og `tools/maint/*` (only for explicit seed maintenance)
...
# FAIL: If any normal command falls back to bin/bootstrap, legacy tools/c_minimal_vm/,
...
### ❌ Hidden fallback (incorrect, removed)
...
5. ✅ Move old bootstrap guidance out of active normal-path docs
