...
def _normalize_token(token: dict[str, Any]) -> dict[str, Any]:
    if token.get("type") == "NUMBER" and isinstance(token.get("value"), int):
        return {**token, "value": str(token["value"])}
    return token


def tokens_from_current_lexer_text(source: str) -> LexerParityResult:
    tokens = [_normalize_token(asdict(t)) for t in lex_source_text(source)]
...
def tokens_from_current_lexer_file(source_file: str) -> LexerParityResult:
    ...
    tokens = [_normalize_token(asdict(t)) for t in lex_source_file(str(source_path))]
