...
class SemanticAnalyzer:
    def __init__(self, alias_map=None):
        ...
        self.functions = {
            ...
            "selfhost.common.native_build_preview": FunctionSymbol("native_build_preview", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.common"),
            "selfhost.common.disasm_uttrykk": FunctionSymbol("disasm_uttrykk", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.common"),
            "selfhost.compiler.disasm_uttrykk": FunctionSymbol("disasm_uttrykk", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_uttrykk_med_miljo": FunctionSymbol("disasm_uttrykk_med_miljo", [TYPE_TEXT, None, None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_skript": FunctionSymbol("disasm_skript", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.instruksjon_til_c": FunctionSymbol("instruksjon_til_c", [TYPE_TEXT, TYPE_INT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.demo_program": FunctionSymbol("demo_program", [], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_til_c": FunctionSymbol("kompiler_til_c", [None, None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_uttrykk_til_c": FunctionSymbol("kompiler_uttrykk_til_c", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_uttrykk_til_c_med_miljo": FunctionSymbol("kompiler_uttrykk_til_c_med_miljo", [TYPE_TEXT, None, None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_skript_til_c": FunctionSymbol("kompiler_skript_til_c", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_fra_linjer": FunctionSymbol("kompiler_fra_linjer", [None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_fra_tokens": FunctionSymbol("kompiler_fra_tokens", [None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_fra_kilde": FunctionSymbol("kompiler_fra_kilde", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.kompiler_fra_kilde_strict": FunctionSymbol("kompiler_fra_kilde_strict", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_fra_tokens": FunctionSymbol("disasm_fra_tokens", [None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_fra_tokens_strict": FunctionSymbol("disasm_fra_tokens_strict", [None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_fra_kilde": FunctionSymbol("disasm_fra_kilde", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_fra_kilde_strict": FunctionSymbol("disasm_fra_kilde_strict", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.op_krever_arg": FunctionSymbol("op_krever_arg", [TYPE_TEXT], TYPE_BOOL, True, "selfhost.compiler"),
            "selfhost.compiler.op_kjent": FunctionSymbol("op_kjent", [TYPE_TEXT], TYPE_BOOL, True, "selfhost.compiler"),
            "selfhost.compiler.er_heltall_token": FunctionSymbol("er_heltall_token", [TYPE_TEXT], TYPE_BOOL, True, "selfhost.compiler"),
            "selfhost.compiler.normaliser_norsk_token": FunctionSymbol("normaliser_norsk_token", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.operator_til_opcode": FunctionSymbol("operator_til_opcode", [TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.stack_behov": FunctionSymbol("stack_behov", [TYPE_TEXT], TYPE_INT, True, "selfhost.compiler"),
            "selfhost.compiler.stack_endring": FunctionSymbol("stack_endring", [TYPE_TEXT], TYPE_INT, True, "selfhost.compiler"),
            "selfhost.compiler.append_linje": FunctionSymbol("append_linje", [TYPE_TEXT, TYPE_TEXT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.instruksjon_til_tekst": FunctionSymbol("instruksjon_til_tekst", [TYPE_TEXT, TYPE_INT], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.disasm_program": FunctionSymbol("disasm_program", [None, None], TYPE_TEXT, True, "selfhost.compiler"),
            "selfhost.compiler.uttrykk_til_ops_og_verdier_med_miljo": FunctionSymbol("uttrykk_til_ops_og_verdier_med_miljo", [None, None, None, None, None], TYPE_TEXT, True, "selfhost.compiler"),
        }
    ...
    def is_list_type(self, type_name):
        return type_name in (TYPE_LIST_INT, TYPE_LIST_TEXT) or (
            isinstance(type_name, str) and type_name.startswith("liste_")
        )

    def list_element_type(self, list_type):
        if list_type == TYPE_LIST_INT:
            return TYPE_INT
        if list_type == TYPE_LIST_TEXT:
            return TYPE_TEXT
        if isinstance(list_type, str) and list_type.startswith("liste_"):
            return list_type[len("liste_"):]
        return None
    ...
    def check_stmt(self, stmt, scope, expected_return_type, in_loop, struct_fields):
        if isinstance(stmt, VarDeclareNode):
            ...
            if stmt.var_type is None:
                ...
                if is_empty_list and self.is_list_type(stmt.var_type):
        ...
        if isinstance(stmt, VarSetNode):
            ...
            if isinstance(stmt.expr, StructLiteralNode):
                ...
                struct_fields.pop(stmt.name, None)
            if is_empty_list and self.is_list_type(scope[stmt.name]):
                return False
        ...
        if isinstance(stmt, IndexSetNode):
            ...
            if self.is_list_type(target_type):
                if idx_type != TYPE_INT:
                    self.error("Listeindeks må være heltall")
                elem_type = self.list_element_type(target_type)
                if elem_type is not None and value_type != elem_type:
                    self.error(f"{target_type} kan bare få verdier av type {elem_type}, men fikk {value_type}")
        ...
        if isinstance(stmt, ForEachNode):
            ...
            loop_scope = dict(scope)
            elem_type = self.list_element_type(list_type)
            if elem_type is not None:
                loop_scope[stmt.item_name] = elem_type
            else:
    ...
    def check_expr(self, expr, scope, field_schemas):
        ...
        if isinstance(expr, ListLiteralNode):
            ...
            if self.is_map_type(first) or isinstance(first, str):
                return f"liste_{first}"
            self.error("Lister støtter bare heltall, tekst eller ordbøker")
        ...
        if isinstance(expr, ListComprehensionNode):
            ...
            source_elem = self.list_element_type(source_type)
            if source_elem is None:
                self.error("Liste-comprehension krever en liste som kilde")
            item_scope = dict(scope)
            item_scope[expr.item_name] = source_elem
            ...
            if isinstance(item_type, str):
                return f"liste_{item_type}"
            self.error("Liste-comprehension må produsere heltall, tekst eller ordbok")
        ...
        if isinstance(expr, IndexNode):
            ...
            if self.is_list_type(target_type):
                if idx_type != TYPE_INT:
                    self.error("Listeindeks må være heltall")
                return self.list_element_type(target_type)
            if target_type == TYPE_TEXT:
                if idx_type != TYPE_INT:
                    self.error("Tekstindeks må være heltall")
                return TYPE_TEXT
            ...
            self.error("[] kan bare brukes på lister, tekst eller ordbøker")
        ...
        if isinstance(expr, SliceNode):
            ...
            if target_type == TYPE_TEXT or self.is_list_type(target_type):
                return target_type
        ...
        if isinstance(expr, IfExprNode):
            ...
            if then_type == else_type:
                return then_type
            if self.is_empty_list_expr(expr.then_expr) and self.is_list_type(else_type):
                return else_type
            if self.is_empty_list_expr(expr.else_expr) and self.is_list_type(then_type):
                return then_type
        ...
        if isinstance(expr, CallNode):
            ...
            if expr.name == "lengde":
                ...
                if arg_type != TYPE_TEXT and not self.is_list_type(arg_type) and not self.is_map_type(arg_type):
                    self.error("lengde støtter bare tekst, lister og ordbøker")
                return TYPE_INT
            ...
            if expr.name == "fjern_indeks":
                ...
                if not self.is_list_type(list_type) or idx_type != TYPE_INT:
            ...
            if expr.name == "sett_inn":
                ...
                elem_type = self.list_element_type(list_type)
                if elem_type is not None and val_type == elem_type:
                    return TYPE_INT
    ...
    def ensure_assignable(self, expected, actual, where, actual_expr=None):
        ...
        if expected != actual:
            if actual_expr is not None and self.is_empty_list_expr(actual_expr) and self.is_list_type(expected):
                return
