#!/usr/bin/env bash
# tools/maint/regen_verify.sh — historisk verifikasjon av deterministisk regen
#
# Krever seed (dist/norscode_native).
# Verifiserer at historisk regen-output blir identisk over to regen-køyringar.
set -euo pipefail
...
printf '=== Regen-verify (L6 historisk determinisme) ===\n\n'
...
printf '\nSamanliknar SHA-256 mellom to historiske regen-køyringar...\n'
OK=1
...
compare "historisk output: norscode_generated.c" \
    "$REGEN_A/maint/c/norscode_generated.c" "$REGEN_B/maint/c/norscode_generated.c"

printf '\n'
if [ "$OK" -eq 1 ]; then
    printf '=== Regen-verify: BESTÅTT (deterministisk historisk regen) ===\n'
    exit 0
fi
