...
static NcVal *nc_builtin_socket_listen(NcVal *host_v, NcVal *port_v) {
    ...
    if (fd < 0) {
        fprintf(stderr, "[nc_native] socket() feilet for %s:%d: errno=%d\n", host, port, errno);
        return nc_int(-1);
    }
    int opt = 1;
    if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (const char*)&opt, sizeof(opt)) < 0) {
        fprintf(stderr, "[nc_native] setsockopt(SO_REUSEADDR) feilet for %s:%d: errno=%d\n", host, port, errno);
    }
    ...
    if (strcmp(host,"0.0.0.0")==0 || strcmp(host,"")==0) {
        ...
        fprintf(stderr, "[nc_native] inet_pton feilet for host=%s port=%d: errno=%d\n", host, port, errno);
        NC_SOCK_CLOSE(fd);
        return nc_int(-1);
    }
    if (bind(fd,(struct sockaddr*)&addr,sizeof(addr))<0) {
        fprintf(stderr, "[nc_native] bind feilet for %s:%d: errno=%d\n", host, port, errno);
        NC_SOCK_CLOSE(fd);
        return nc_int(-1);
    }
    if (listen(fd,128)<0) {
        fprintf(stderr, "[nc_native] listen feilet for %s:%d: errno=%d\n", host, port, errno);
        NC_SOCK_CLOSE(fd);
        return nc_int(-1);
    }
...
NcVal *g_current_route_handlers = NULL;
NcVal *g_current_functions = NULL;
NcVal *g_current_ncb = NULL;
...
/* ── Sort-hjelperar for std.liste.* ── */
static int nc_list_cmp_int(const void *a, const void *b) {
    NcVal *va=*(NcVal**)a, *vb=*(NcVal**)b;
    long long ia=va&&va->type==NC_INT?va->i:0, ib=vb&&vb->type==NC_INT?vb->i:0;
    return (ia>ib)-(ia<ib);
}
static int nc_list_cmp_str(const void *a, const void *b) {
    NcVal *va=*(NcVal**)a, *vb=*(NcVal**)b;
    const char *sa=va&&va->type==NC_STR?va->s:"", *sb=vb&&vb->type==NC_STR?vb->s:"";
    return strcmp(sa,sb);
}
...
static NcVal *nc_exec_call(NcVal *functions, const char *fn_name, NcVal **args, int nargs, int depth) {
    ...
    while (ip < code_v->list->len) {
        ...
        if (!strcmp(op, "PUSH_CONST")) {
            ...
            else if (!strcmp(cn,"ends_with")||!strcmp(cn,"tekst_slutter_med")||!strcmp(cn,"std.tekst.slutter_med"))   fn_r=nc_builtin_ends_with(narg>0?cargs[0]:nc_nil(),narg>1?cargs[1]:nc_nil());
            ...
            else if (!strcmp(cn,"trim")||!strcmp(cn,"tekst_trim")||!strcmp(cn,"std.tekst.trim"))     fn_r=nc_builtin_trim(narg>0?cargs[0]:nc_nil());
            ...
            else if (!strcmp(cn,"starts_with")||!strcmp(cn,"tekst_starter_med")||!strcmp(cn,"std.tekst.starter_med")||!strcmp(cn,"builtin.std.tekst.starter_med")) fn_r=nc_builtin_starts_with(narg>0?cargs[0]:nc_nil(),narg>1?cargs[1]:nc_nil());
            else if (!strcmp(cn,"contains")||!strcmp(cn,"tekst_inneholder")||!strcmp(cn,"std.tekst.inneholder")||!strcmp(cn,"builtin.std.tekst.inneholder")) fn_r=nc_builtin_contains(narg>0?cargs[0]:nc_nil(),narg>1?cargs[1]:nc_nil());
            else if (!strcmp(cn,"t.hilsen")||!strcmp(cn,"std.tekst.hilsen")) {
            ...
            else if (!strcmp(cn,"t.rop")||!strcmp(cn,"std.tekst.rop")) {
                char *s = nc_to_str_raw(narg>0 ? cargs[0] : nc_nil());
            ...
            else if (!strcmp(cn,"std.json.stringify"))
                fn_r=nc_builtin_json_stringify_smart(narg>0?cargs[0]:nc_nil());
            else if (!strcmp(cn,"json_stringify") || !strcmp(cn,"builtin.json.stringify"))
                fn_r=nc_builtin_json_stringify(narg>0?cargs[0]:nc_nil());
            ...
            else if (!strcmp(cn,"fil_finnes")||!strcmp(cn,"std.fil.finnes"))       fn_r=nc_builtin_fil_finnes(narg>0?cargs[0]:nc_nil());
            ...
            /* ── TCP socket-builtins ── */
            else if (!strcmp(cn,"std.tcp.ny_server") || !strcmp(cn,"tcp.ny_server")) {
                fn_r = nc_builtin_socket_listen(narg>0?cargs[0]:nc_nil(), narg>1?cargs[1]:nc_nil());
            }
            else if (!strcmp(cn,"std.tcp.lytt") || !strcmp(cn,"tcp.lytt")) {
            ...
            else if (!strcmp(cn,"db.create") || !strcmp(cn,"builtin.db.create") || !strcmp(cn,"db_create") || !strcmp(cn,"builtin.db_create")) {
                NcVal *path_v = narg>0 ? cargs[0] : nc_nil();
                char *path = nc_to_str_raw(path_v);
                NcVal *res = nc_map_new();
                if (!path || path[0]=='\0') {
                    nc_index_set(res, nc_str("feilet"), nc_bool(1));
                    nc_index_set(res, nc_str("melding"), nc_str("Mangler databasefil."));
                    fn_r = res;
                }
                else {
                    NcVal *ex = nc_builtin_fil_finnes(path_v);
                    if (nc_truthy(ex)) {
                        char msg[512];
                        snprintf(msg, sizeof(msg), "Databasefil finnes allerede: %s", path);
                        nc_index_set(res, nc_str("feilet"), nc_bool(1));
                        nc_index_set(res, nc_str("melding"), nc_str(msg));
                        fn_r = res;
                    } else {
                        NcVal *wr = nc_builtin_fil_skriv(path_v, nc_str("{}"));
                        if (!wr || wr->type == NC_NIL) {
                            char msg[512];
                            snprintf(msg, sizeof(msg), "Database opprettet: %s", path);
                            nc_index_set(res, nc_str("feilet"), nc_bool(0));
                            nc_index_set(res, nc_str("verdi"), nc_str(msg));
                            fn_r = res;
                        } else {
                            nc_index_set(res, nc_str("feilet"), nc_bool(1));
                            nc_index_set(res, nc_str("melding"), nc_str("Kunne ikke opprette database."));
                            fn_r = res;
                        }
                    }
                }
            ...
            else if (!strcmp(cn,"std.json.hent_tekst")||!strcmp(cn,"builtin.std.json.hent_tekst")) fn_r=(narg>=2)?nc_index_get(cargs[0],cargs[1]):nc_nil();
            else if (!strcmp(cn,"std.liste.første_tall")||!strcmp(cn,"builtin.std.liste.første_tall")) { NcVal *l=narg>0?cargs[0]:nc_nil(); fn_r=(l&&l->type==NC_LIST&&l->list->len>0)?l->list->items[0]:nc_int(0); }
            else if (!strcmp(cn,"std.liste.første_tekst")||!strcmp(cn,"builtin.std.liste.første_tekst")) { NcVal *l=narg>0?cargs[0]:nc_nil(); fn_r=(l&&l->type==NC_LIST&&l->list->len>0)?l->list->items[0]:nc_str(""); }
            else if (!strcmp(cn,"std.ordbok.antall_tall")||!strcmp(cn,"builtin.std.ordbok.antall_tall")) { NcVal *m=narg>0?cargs[0]:nc_nil(); fn_r=nc_int(m&&m->type==NC_MAP?m->map->len:0); }
            else if (!strcmp(cn,"std.ordbok.har_tall")||!strcmp(cn,"std.ordbok.har_tekst")||!strcmp(cn,"builtin.std.ordbok.har_tall")) fn_r=nc_builtin_finnes_nokkel(narg>0?cargs[0]:nc_nil(),narg>1?cargs[1]:nc_nil());
            else if (!strcmp(cn,"std.ordbok.sett_tall")||!strcmp(cn,"builtin.std.ordbok.sett_tall")) { if(narg>=3) nc_index_set(cargs[0],cargs[1],cargs[2]); fn_r=nc_int(narg>0&&cargs[0]&&cargs[0]->type==NC_MAP?cargs[0]->map->len:0); }
            else if (!strcmp(cn,"std.lagring.sett_tekst")||!strcmp(cn,"builtin.std.lagring.sett_tekst")) { if(narg>=3) nc_index_set(cargs[0],cargs[1],cargs[2]); fn_r=nc_int(0); }
            else if (!strcmp(cn,"std.lagring.sett_tall")||!strcmp(cn,"builtin.std.lagring.sett_tall")) { if(narg>=3) nc_index_set(cargs[0],cargs[1],nc_to_str(cargs[2])); fn_r=nc_int(0); }
            else if (!strcmp(cn,"std.fil.skriv_fil")||!strcmp(cn,"builtin.std.fil.skriv_fil")) { if(narg>=2) nc_builtin_fil_skriv(cargs[0],cargs[1]); fn_r=nc_nil(); }
            else if (!strcmp(cn,"std.db.open")||!strcmp(cn,"builtin.std.db.open")) { char *p=nc_to_str_raw(narg>0?cargs[0]:nc_nil()); sqlite3 *db=NULL; fn_r=(sqlite3_open(p,&db)==SQLITE_OK)?nc_db_make_handle(db,-1):nc_nil(); free(p); }
            else if (!strcmp(cn,"std.db.create")||!strcmp(cn,"builtin.std.db.create")) {
                NcVal *path_v = narg>0 ? cargs[0] : nc_nil();
                char *path = nc_to_str_raw(path_v);
                NcVal *res = nc_map_new();
                if (!path || path[0]=='\0') {
            ...
            /* ── std.db.* (alias for db.*) ── */
            else if (!strcmp(cn,"std.db.ping")) fn_r=nc_bool(nc_db_handle_ptr(narg>0?cargs[0]:nc_nil())!=NULL);
            else if (!strcmp(cn,"std.db.close")) { NcVal *h=narg>0?cargs[0]:nc_nil(); sqlite3 *db=nc_db_handle_ptr(h); if (!db) { fn_r=nc_bool(0); } else { NcVal *pv=nc_index_get(h,nc_str("__pool")); if (pv&&pv->type==NC_INT&&pv->i>=0&&(int)pv->i<g_nc_db_pool_count) { int _idx2=(int)pv->i; if (g_nc_db_pools[_idx2].avail<NC_DB_MAX_POOL_CONNS) g_nc_db_pools[_idx2].conns[g_nc_db_pools[_idx2].avail++]=db; else sqlite3_close(db); } else sqlite3_close(db); fn_r=nc_bool(1); } }
            else if (!strcmp(cn,"std.db.execute")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); if (!db){fn_r=nc_int(-1);}else{char *s=nc_to_str_raw(narg>1?cargs[1]:nc_nil());char *e=NULL;sqlite3_exec(db,s,NULL,NULL,&e);free(s);if(e){sqlite3_free(e);fn_r=nc_int(-1);}else fn_r=nc_int(sqlite3_changes(db));} }
            else if (!strcmp(cn,"std.db.query_text")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); fn_r=nc_str(""); if(db){char *s=nc_to_str_raw(narg>1?cargs[1]:nc_nil());sqlite3_stmt *st=NULL;if(sqlite3_prepare_v2(db,s,-1,&st,NULL)==SQLITE_OK){if(sqlite3_step(st)==SQLITE_ROW){const unsigned char *t=sqlite3_column_text(st,0);if(t)fn_r=nc_str((const char*)t);}sqlite3_finalize(st);}free(s);} }
            else if (!strcmp(cn,"std.db.query_int")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); fn_r=nc_int(0); if(db){char *s=nc_to_str_raw(narg>1?cargs[1]:nc_nil());sqlite3_stmt *st=NULL;if(sqlite3_prepare_v2(db,s,-1,&st,NULL)==SQLITE_OK){if(sqlite3_step(st)==SQLITE_ROW)fn_r=nc_int((long long)sqlite3_column_int64(st,0));sqlite3_finalize(st);}free(s);} }
            else if (!strcmp(cn,"std.db.begin")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); char *_e=NULL; fn_r=(db&&sqlite3_exec(db,"BEGIN",NULL,NULL,&_e)==SQLITE_OK)?nc_bool(1):nc_bool(0); if(_e)sqlite3_free(_e); }
            else if (!strcmp(cn,"std.db.commit")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); char *_e=NULL; fn_r=(db&&sqlite3_exec(db,"COMMIT",NULL,NULL,&_e)==SQLITE_OK)?nc_bool(1):nc_bool(0); if(_e)sqlite3_free(_e); }
            else if (!strcmp(cn,"std.db.rollback")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); char *_e=NULL; fn_r=(db&&sqlite3_exec(db,"ROLLBACK",NULL,NULL,&_e)==SQLITE_OK)?nc_bool(1):nc_bool(0); if(_e)sqlite3_free(_e); }
            else if (!strcmp(cn,"std.db.migrate")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); fn_r=nc_int(0); if(db){char *s=nc_to_str_raw(narg>1?cargs[1]:nc_nil()); sqlite3_exec(db,"CREATE TABLE IF NOT EXISTS __nc_migrations(hash TEXT PRIMARY KEY)",NULL,NULL,NULL); char hstr[24]; snprintf(hstr,sizeof(hstr),"%llu",nc_db_sql_hash(s)); char qsql[80]; snprintf(qsql,sizeof(qsql),"SELECT 1 FROM __nc_migrations WHERE hash='%s'",hstr); sqlite3_stmt *chk=NULL; int already=0; if(sqlite3_prepare_v2(db,qsql,-1,&chk,NULL)==SQLITE_OK){if(sqlite3_step(chk)==SQLITE_ROW)already=1;sqlite3_finalize(chk);} if(!already){char *e=NULL;int rc=sqlite3_exec(db,s,NULL,NULL,&e);if(!e&&rc==SQLITE_OK){char isql[80];snprintf(isql,sizeof(isql),"INSERT INTO __nc_migrations VALUES('%s')",hstr);sqlite3_exec(db,isql,NULL,NULL,NULL);fn_r=nc_int(1);}else if(e)sqlite3_free(e);} free(s);} }
            else if (!strcmp(cn,"std.db.transaction")) { sqlite3 *db=nc_db_handle_ptr(narg>0?cargs[0]:nc_nil()); fn_r=nc_int(0); if(db){char *s=nc_to_str_raw(narg>1?cargs[1]:nc_nil());char *e=NULL;sqlite3_exec(db,"BEGIN",NULL,NULL,NULL);int rc=sqlite3_exec(db,s,NULL,NULL,&e);free(s);if(e||rc!=SQLITE_OK){if(e)sqlite3_free(e);sqlite3_exec(db,"ROLLBACK",NULL,NULL,NULL);}else{fn_r=nc_int(sqlite3_changes(db));sqlite3_exec(db,"COMMIT",NULL,NULL,NULL);}} }
            else if (!strcmp(cn,"std.db.pool")) { char *p=nc_to_str_raw(narg>0?cargs[0]:nc_nil()); int sz=narg>1&&cargs[1]&&cargs[1]->type==NC_INT?(int)cargs[1]->i:1; int _idx3=g_nc_db_pool_count++; if(_idx3<NC_DB_MAX_POOLS){strncpy(g_nc_db_pools[_idx3].path,p,511);g_nc_db_pools[_idx3].avail=0;sqlite3 *_db3=NULL;if(sqlite3_open(p,&_db3)==SQLITE_OK)g_nc_db_pools[_idx3].conns[g_nc_db_pools[_idx3].avail++]=_db3;NcVal *m=nc_map_new();nc_index_set(m,nc_str("__pool"),nc_int(_idx3));(void)sz;fn_r=m;}else fn_r=nc_nil(); free(p); }
            else if (!strcmp(cn,"std.db.pool_size")) { NcVal *p=narg>0?cargs[0]:nc_nil(); fn_r=nc_int(0); if(p&&p->type==NC_MAP){NcVal *iv=nc_index_get(p,nc_str("__pool"));if(iv&&iv->type==NC_INT&&iv->i<g_nc_db_pool_count)fn_r=nc_int(g_nc_db_pools[(int)iv->i].avail);} }
            else if (!strcmp(cn,"std.db.pool_acquire")) { NcVal *pool=narg>0?cargs[0]:nc_nil(); fn_r=nc_nil(); if(pool&&pool->type==NC_MAP){NcVal *iv=nc_index_get(pool,nc_str("__pool"));if(iv&&iv->type==NC_INT){int _idx4=(int)iv->i;if(_idx4<g_nc_db_pool_count){sqlite3 *_db4=NULL;if(g_nc_db_pools[_idx4].avail>0)_db4=g_nc_db_pools[_idx4].conns[--g_nc_db_pools[_idx4].avail];else sqlite3_open(g_nc_db_pools[_idx4].path,&_db4);if(_db4)fn_r=nc_db_make_handle(_db4,_idx4);}}} }
            else if (!strcmp(cn,"std.db.pool_close")) { NcVal *pool=narg>0?cargs[0]:nc_nil(); fn_r=nc_bool(0); if(pool&&pool->type==NC_MAP){NcVal *iv=nc_index_get(pool,nc_str("__pool"));if(iv&&iv->type==NC_INT&&iv->i<g_nc_db_pool_count){int _idx5=(int)iv->i;for(int _i=0;_i<g_nc_db_pools[_idx5].avail;_i++)sqlite3_close(g_nc_db_pools[_idx5].conns[_i]);g_nc_db_pools[_idx5].avail=0;fn_r=nc_bool(1);}} }
            /* ── std.fil.* ── */
            else if (!strcmp(cn,"std.fil.les")||!strcmp(cn,"fil.les")) fn_r=nc_builtin_fil_les(narg>0?cargs[0]:nc_nil());
            else if (!strcmp(cn,"std.fil.append")||!strcmp(cn,"fil.append")) { if(narg>=2) nc_builtin_fil_append(cargs[0],cargs[1]); fn_r=nc_nil(); }
            /* ── std.json typed helpers ── */
            else if (!strcmp(cn,"std.json.hent_tall")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                if(v&&v->type==NC_INT) fn_r=v;
                else { char *sv=v?nc_to_str_raw(v):NULL; fn_r=nc_int(sv?atoll(sv):0); if(sv)free(sv); }
            }
            else if (!strcmp(cn,"std.json.hent_bool")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                if(!v||v->type==NC_NIL) fn_r=nc_bool(0);
                else if(v->type==NC_BOOL) fn_r=v;
                else if(v->type==NC_INT) fn_r=nc_bool(v->i!=0);
                else { char *sv=nc_to_str_raw(v); fn_r=nc_bool(strcmp(sv,"false")!=0&&strcmp(sv,"usann")!=0&&strcmp(sv,"0")!=0&&sv[0]!='\0'); free(sv); }
            }
            else if (!strcmp(cn,"std.json.hent_array_tekst")) {
                NcVal *sv=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                fn_r=nc_list_new();
                if(sv&&sv->type==NC_STR) { NcVal *p=nc_builtin_json_parse_raw(sv); if(p&&p->type==NC_LIST)fn_r=p; }
            }
            else if (!strcmp(cn,"std.json.hent_array_tall")) {
                NcVal *sv=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                fn_r=nc_list_new();
                if(sv&&sv->type==NC_STR) { NcVal *p=nc_builtin_json_parse_raw(sv); if(p&&p->type==NC_LIST){for(int _i=0;_i<p->list->len;_i++){NcVal *it=p->list->items[_i];if(it&&it->type!=NC_INT){char *s=nc_to_str_raw(it);p->list->items[_i]=nc_int(atoll(s));free(s);}}fn_r=p;} }
            }
            /* ── std.liste.* ── */
            else if (!strcmp(cn,"std.liste.antall_tall")||!strcmp(cn,"std.liste.antall_tekst")) fn_r=nc_int(narg>0&&cargs[0]&&cargs[0]->type==NC_LIST?cargs[0]->list->len:0);
            else if (!strcmp(cn,"std.liste.inneholder_tall")||!strcmp(cn,"std.liste.inneholder_tekst")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil(),*val=narg>1?cargs[1]:nc_nil(); fn_r=nc_bool(0);
                if(lst&&lst->type==NC_LIST) for(int _i=0;_i<lst->list->len;_i++) if(nc_eq(lst->list->items[_i],val)){fn_r=nc_bool(1);break;}
            }
            else if (!strcmp(cn,"std.liste.sorter_tall")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil();
                if(lst&&lst->type==NC_LIST) qsort(lst->list->items,lst->list->len,sizeof(NcVal*),nc_list_cmp_int);
                fn_r=nc_int(lst&&lst->type==NC_LIST?lst->list->len:0);
            }
            else if (!strcmp(cn,"std.liste.sorter_tekst")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil();
                if(lst&&lst->type==NC_LIST) qsort(lst->list->items,lst->list->len,sizeof(NcVal*),nc_list_cmp_str);
                fn_r=nc_int(lst&&lst->type==NC_LIST?lst->list->len:0);
            }
            else if (!strcmp(cn,"std.liste.summer_tall")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil(); long long sum=0;
                if(lst&&lst->type==NC_LIST) for(int _i=0;_i<lst->list->len;_i++){NcVal *it=lst->list->items[_i];if(it&&it->type==NC_INT)sum+=it->i;}
                fn_r=nc_int(sum);
            }
            else if (!strcmp(cn,"std.liste.maks_tall")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil(); long long mx=0; int first=1;
                if(lst&&lst->type==NC_LIST) for(int _i=0;_i<lst->list->len;_i++){NcVal *it=lst->list->items[_i];if(it&&it->type==NC_INT){if(first||it->i>mx){mx=it->i;first=0;}}}
                fn_r=nc_int(mx);
            }
            else if (!strcmp(cn,"std.liste.min_tall")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil(); long long mn=0; int first=1;
                if(lst&&lst->type==NC_LIST) for(int _i=0;_i<lst->list->len;_i++){NcVal *it=lst->list->items[_i];if(it&&it->type==NC_INT){if(first||it->i<mn){mn=it->i;first=0;}}}
                fn_r=nc_int(mn);
            }
            else if (!strcmp(cn,"std.liste.omvendt_tall")||!strcmp(cn,"std.liste.omvendt_tekst")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil();
                if(lst&&lst->type==NC_LIST){int n=lst->list->len;for(int _i=0;_i<n/2;_i++){NcVal *tmp=lst->list->items[_i];lst->list->items[_i]=lst->list->items[n-1-_i];lst->list->items[n-1-_i]=tmp;}}
                fn_r=nc_int(lst&&lst->type==NC_LIST?lst->list->len:0);
            }
            else if (!strcmp(cn,"std.liste.finn_indeks_tall")||!strcmp(cn,"std.liste.finn_indeks_tekst")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil(),*val=narg>1?cargs[1]:nc_nil(); fn_r=nc_int(-1);
                if(lst&&lst->type==NC_LIST) for(int _i=0;_i<lst->list->len;_i++) if(nc_eq(lst->list->items[_i],val)){fn_r=nc_int(_i);break;}
            }
            else if (!strcmp(cn,"std.liste.fjern_alle_tall")||!strcmp(cn,"std.liste.fjern_alle_tekst")) {
                NcVal *lst=narg>0?cargs[0]:nc_nil(),*val=narg>1?cargs[1]:nc_nil();
                if(lst&&lst->type==NC_LIST){int w=0;for(int _i=0;_i<lst->list->len;_i++)if(!nc_eq(lst->list->items[_i],val))lst->list->items[w++]=lst->list->items[_i];lst->list->len=w;}
                fn_r=nc_int(lst&&lst->type==NC_LIST?lst->list->len:0);
            }
            /* ── std.ordbok.* ── */
            else if (!strcmp(cn,"std.ordbok.hent_tall")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                if(v&&v->type==NC_INT) fn_r=v;
                else { char *sv=v?nc_to_str_raw(v):NULL; fn_r=nc_int(sv?atoll(sv):0); if(sv)free(sv); }
            }
            else if (!strcmp(cn,"std.ordbok.hent_tekst")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                fn_r=(v&&v->type==NC_STR)?v:nc_str("");
            }
            else if (!strcmp(cn,"std.ordbok.sett_bool")||!strcmp(cn,"std.ordbok.har_bool")) {
                if(!strcmp(cn,"std.ordbok.sett_bool")) { if(narg>=3) nc_index_set(cargs[0],cargs[1],nc_bool(nc_truthy(cargs[2]))); fn_r=nc_int(0); }
                else fn_r=nc_builtin_finnes_nokkel(narg>0?cargs[0]:nc_nil(),narg>1?cargs[1]:nc_nil());
            }
            else if (!strcmp(cn,"std.ordbok.hent_bool")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                if(!v||v->type==NC_NIL) fn_r=nc_bool(0);
                else if(v->type==NC_BOOL) fn_r=v;
                else if(v->type==NC_INT) fn_r=nc_bool(v->i!=0);
            ...
            else if (!strcmp(cn,"std.ordbok.fjern_bool")||!strcmp(cn,"std.ordbok.fjern_tall")||!strcmp(cn,"std.ordbok.fjern_tekst")) {
                if(narg>=2) nc_builtin_fjern_nokkel(cargs[0],cargs[1]); fn_r=nc_int(0);
            }
            /* ── std.lagring.* ── */
            else if (!strcmp(cn,"std.lagring.sett_bool")||!strcmp(cn,"lagring.sett_bool")) {
                if(narg>=3) { int bv=nc_truthy(cargs[2]); nc_index_set(cargs[0],cargs[1],nc_str(bv?"sann":"usann")); } fn_r=nc_int(0);
            }
            else if (!strcmp(cn,"std.lagring.hent_tall")||!strcmp(cn,"lagring.hent_tall")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                if(v&&v->type==NC_INT) fn_r=v;
                else { char *sv=v?nc_to_str_raw(v):NULL; fn_r=nc_int(sv?atoll(sv):0); if(sv)free(sv); }
            }
            else if (!strcmp(cn,"std.lagring.hent_tekst")||!strcmp(cn,"lagring.hent_tekst")||!strcmp(cn,"builtin.lagring.hent_tekst")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                fn_r=(v&&v->type==NC_STR)?v:nc_str("");
            }
            else if (!strcmp(cn,"std.lagring.hent_bool")||!strcmp(cn,"lagring.hent_bool")) {
                NcVal *v=narg>=2?nc_index_get(cargs[0],cargs[1]):nc_nil();
                if(!v||v->type==NC_NIL) fn_r=nc_bool(0);
                else if(v->type==NC_BOOL) fn_r=v;
                else if(v->type==NC_INT) fn_r=nc_bool(v->i!=0);
            ...
            else if (!strcmp(cn,"std.lagring.lagre")||!strcmp(cn,"lagring.lagre")||!strcmp(cn,"builtin.lagring.lagre")) {
                int _lagre_caught = 0;
                if (try_depth > 0) {
                    jmp_buf _lagre_saved; memcpy(&_lagre_saved,&g_err_jmp,sizeof(jmp_buf));
                    if (setjmp(g_err_jmp)) {
                        memcpy(&g_err_jmp,&_lagre_saved,sizeof(jmp_buf));
                        if (try_depth > 0) {
                            { const char *_em=g_err_msg;
                              if (strncmp(_em,"Norscode unntak: ",17)==0) _em+=17;
                              else if (strncmp(_em,"nc-vm feil: Norscode unntak: ",28)==0) _em+=28;
                            ...
                            sp=try_stack[try_depth-1].sp_depth;
                            NcVal *_tgt=nc_index_get(label_map,nc_str(_cl));
                            if (_tgt&&_tgt->type==NC_INT){ip=(int)_tgt->i+1;_lagre_caught=1;}
                        }
                        fn_r=nc_nil();
                    } else {
                        NcVal *sti_v=narg>0?cargs[0]:nc_nil(), *map_v=narg>1?cargs[1]:nc_nil();
                        char *sti_s=nc_to_str_raw(sti_v);
                        NcVal *json_v=nc_builtin_json_stringify(map_v);
                        char *json_s=json_v&&json_v->type==NC_STR?strdup(json_v->s):strdup("{}");
                        FILE *_fp=fopen(sti_s,"w");
                        if(!_fp){free(sti_s);free(json_s);nc_throw("IOFeil: lagring.lagre feilet: kan ikkje opne for skrive");}
                        int _ok=(fputs(json_s,_fp)>=0);
                        fclose(_fp); free(json_s); free(sti_s);
                        if(!_ok) nc_throw("IOFeil: lagring.lagre feilet: kunne ikkje skrive");
                        fn_r=nc_int(map_v&&map_v->type==NC_MAP?map_v->map->len:0);
                        memcpy(&g_err_jmp,&_lagre_saved,sizeof(jmp_buf));
                    }
                } else {
                    NcVal *sti_v=narg>0?cargs[0]:nc_nil(), *map_v=narg>1?cargs[1]:nc_nil();
                ...
                if (_lagre_caught) { free(cargs); free(callee); continue; }
            }
            else if (!strcmp(cn,"std.lagring.last")||!strcmp(cn,"lagring.last")||!strcmp(cn,"builtin.lagring.last")) {
                NcVal *sti_v=narg>0?cargs[0]:nc_nil();
                NcVal *content_v=nc_builtin_fil_les(sti_v);
                if(!content_v||content_v->type==NC_NIL||(content_v->type==NC_STR&&!content_v->s[0])) fn_r=nc_map_new();
                else { NcVal *p=nc_builtin_json_parse_raw(content_v); fn_r=(p&&p->type==NC_MAP)?p:nc_map_new(); }
            }
            else if (!strcmp(cn,"std.lagring.fjern")||!strcmp(cn,"lagring.fjern")||!strcmp(cn,"builtin.lagring.fjern")) {
                if(narg>=2) nc_builtin_fjern_nokkel(cargs[0],cargs[1]);
                fn_r=nc_int(narg>0&&cargs[0]&&cargs[0]->type==NC_MAP?cargs[0]->map->len:0);
            }
            ...
            else if (!strcmp(cn,"json.parse")||!strcmp(cn,"json_parse")||!strcmp(cn,"std.json.parse")) fn_r=nc_builtin_json_parse_norscode(narg>0?cargs[0]:nc_nil());
            else if (!strcmp(cn,"json.parse_raw")||!strcmp(cn,"json_parse_raw")) fn_r=nc_builtin_json_parse_raw(narg>0?cargs[0]:nc_nil());
            else if (!strcmp(cn,"json.stringify")||!strcmp(cn,"std.json.stringify")) fn_r=nc_builtin_json_stringify_smart(narg>0?cargs[0]:nc_nil());
            ...
            else if (!strcmp(cn,"lagring.sett_tekst") || !strcmp(cn,"builtin.lagring.sett_tekst") || !strcmp(cn,"std.lagring.sett_tekst")) { if(narg>=3) nc_index_set(cargs[0], cargs[1], cargs[2]); fn_r = nc_nil(); }
            ...
            /* openapi_json er implementert i std/web.no — ikkje C-stub her */
                ...
                /* builtin.path.* dispatchers (same handlers as std.path.*) */
                    ...
                    NcVal *req_id = nc_builtin_ncb_next_request_id(NULL, 0);
                    ...
                    nc_index_set(fn_r, nc_str("__request_id__"), req_id);
                                    ...
                                    *space = 0;
                                    ...
                                    if (!strcmp(spec_method_up, method_up)) {
                                        ...
                                        if (!strcmp(spec_path, path)) {
                                            ...
                                            char *pp = strdup(path);
                                            char *sp_save = NULL, *pp_save = NULL;
                                            char *sp_tok = strtok_r(sp, "/", &sp_save);
                                            char *pp_tok = strtok_r(pp, "/", &pp_save);
                                            int ok = 1;
                                            ...
                                            while (sp_tok || pp_tok) {
                                                ...
                                                if (sp_tok[0] == '{') {
                                                    ...
                                                    *endb = 0; *colon = 0;
                                                    ...
                                                    const char *param_type = colon + 1;
                                                    if (!strcmp(param_type, "int")) {
                                                        char *_ep; strtol(pp_tok, &_ep, 10);
                                                        if (_ep == pp_tok || *_ep != '\0') { ok = 0; break; }
                                                    }
                                                    nc_index_set(params, nc_str(param_name), nc_str(pp_tok));
                                                } else if (strcmp(sp_tok, pp_tok) != 0) {
                                                    ok = 0; break;
                                                }
                                                sp_tok = strtok_r(NULL, "/", &sp_save);
                                                pp_tok = strtok_r(NULL, "/", &pp_save);
                                            }
                                            if (ok && !sp_tok && !pp_tok) {
                                                path_matched = 1;
                    ...
                    if (resp && resp->type == NC_MAP) {
                        ...
                        if (!v || v->type == NC_NIL) v = nc_index_get(resp, nc_str("headers"));
                        fn_r = (v && v->type != NC_NIL) ? v : nc_map_new();
                    } else {
                        fn_r = nc_map_new();
                    }
                }
                else if (strstr(cn, "request_param_int")) {
                    /* web.request_param_int(ctx, name) -> int or -1 */
                    NcVal *ctx = narg > 0 ? cargs[0] : nc_nil();
                    NcVal *key = narg > 1 ? cargs[1] : nc_str("");
                    fn_r = nc_int(-1);
                    if (ctx && ctx->type == NC_MAP && key && key->type == NC_STR) {
                        NcVal *params = nc_index_get(ctx, nc_str("__params__"));
                        if (params && params->type == NC_MAP) {
                            NcVal *v = nc_index_get(params, key);
                            if (v && v->type == NC_STR) { fn_r = nc_int(atoll(v->s)); }
                            else if (v && v->type == NC_INT) { fn_r = nc_int((int)v->i); }
                        }
                    }
                }
                else if (strstr(cn, "request_param")) {
                    /* web.request_param(ctx, name) -> tekst or "" */
                    NcVal *ctx = narg > 0 ? cargs[0] : nc_nil();
                    NcVal *key = narg > 1 ? cargs[1] : nc_str("");
                    fn_r = nc_str("");
                    if (ctx && ctx->type == NC_MAP && key && key->type == NC_STR) {
                        ...
                        if (params && params->type == NC_MAP) {
                            NcVal *v = nc_index_get(params, key);
                            if (v && v->type == NC_STR) { fn_r = v; }
                            else if (v) { char *s = nc_to_str_raw(v); fn_r = nc_str(s); free(s); }
                        }
                    }
                }
                    ...
                    if (ctx && ctx->type == NC_MAP) {
                        ...
                        if (!v || v->type == NC_NIL) v = nc_index_get(ctx, nc_str("body"));
                        fn_r = (v && v->type == NC_STR) ? v : nc_str("");
                    } else {
                        fn_r = nc_str("");
                    }
                    ...
                    if (ctx && ctx->type == NC_MAP) {
                        ...
                        if (!headers || headers->type == NC_NIL || headers->type != NC_MAP) headers = nc_index_get(ctx, nc_str("headers"));
                    ...
                    if (ctx && ctx->type == NC_MAP) {
                        ...
                        if (!q || q->type == NC_NIL) q = nc_index_get(ctx, nc_str("query"));
                    ...
                    if (ctx && ctx->type == NC_MAP) {
                        ...
                        if (!v || v->type == NC_NIL) v = nc_index_get(ctx, nc_str("method"));
                        fn_r = (v && v->type == NC_STR) ? v : nc_str("");
                    } else {
                        fn_r = nc_str("");
                    }
                    ...
                    if (ctx && ctx->type == NC_MAP && rolle && rolle->type == NC_STR) {
                        ...
                        if (headers && headers->type == NC_MAP) {
                            ...
                            if (!v || v->type == NC_NIL) v = nc_index_get(headers, nc_str("x-roles"));
                            if (v && v->type == NC_STR) {
                    ...
                    if (resp && resp->type == NC_MAP) {
                        ...
                        if (!headers || headers->type == NC_NIL || headers->type != NC_MAP) headers = nc_index_get(resp, nc_str("headers"));
                        if (headers && headers->type == NC_MAP) {
                            NcVal *low_v = nc_builtin_lower(key);
                            char *low = low_v ? nc_to_str_raw(low_v) : NULL;
                            NcVal *v = nc_index_get(headers, nc_str(low ? low : ""));
                            if (!v || v->type == NC_NIL) v = nc_index_get(headers, key);
                            fn_r = (v && v->type == NC_STR) ? v : nc_str("");
                            free(low);
                        } else {
                            fn_r = nc_str("");
                ...
                else if (strstr(cn, "request_id")) {
                    NcVal *ctx = narg > 0 ? cargs[0] : nc_nil();
                    if (ctx && ctx->type == NC_MAP) {
                        NcVal *v = nc_index_get(ctx, nc_str("__request_id__"));
                        fn_r = (v && v->type == NC_STR) ? v : nc_str("req-0");
                    } else { fn_r = nc_str("req-0"); }
                }
                else if (strstr(cn, "request_path")) {
                    NcVal *ctx = narg > 0 ? cargs[0] : nc_nil();
                    if (ctx && ctx->type == NC_MAP) {
                        NcVal *v = nc_index_get(ctx, nc_str("__path__"));
                        if (!v || v->type == NC_NIL) v = nc_index_get(ctx, nc_str("path"));
                        fn_r = (v && v->type == NC_STR) ? v : nc_str("");
                    } else { fn_r = nc_str(""); }
                }
                /* std.trace.* handlers */
                else if (strstr(cn, "trace.start")) {
                    /* trace.start(name, trace_id, parent) -> spor map with 7 keys */
                    NcVal *spor = nc_map_new();
                    nc_index_set(spor, nc_str("name"),       narg>0 ? cargs[0] : nc_str(""));
                    nc_index_set(spor, nc_str("trace_id"),   narg>1 ? cargs[1] : nc_str(""));
                    nc_index_set(spor, nc_str("parent"),     narg>2 ? cargs[2] : nc_str(""));
                    nc_index_set(spor, nc_str("events"),     nc_list_new());
                    nc_index_set(spor, nc_str("fields"),     nc_map_new());
                    nc_index_set(spor, nc_str("status"),     nc_str("running"));
                    nc_index_set(spor, nc_str("start_time"), nc_str("0"));
                    fn_r = spor;
                }
                else if (strstr(cn, "trace.koble_request")) {
                    /* trace.koble_request(spor, request_id, method, path) -> key count */
                    NcVal *spor = narg>0 ? cargs[0] : nc_nil();
                    if (spor && spor->type == NC_MAP) {
                        if (narg>1) nc_index_set(spor, nc_str("request_id"), cargs[1]);
                        if (narg>2) nc_index_set(spor, nc_str("method"),     cargs[2]);
                        if (narg>3) nc_index_set(spor, nc_str("path"),       cargs[3]);
                        fn_r = nc_int(spor->map->len);
                    } else { fn_r = nc_int(0); }
                }
                else if (strstr(cn, "trace.event")) {
                    /* trace.event(spor, name, status) -> event count */
                    NcVal *spor = narg>0 ? cargs[0] : nc_nil();
                    if (spor && spor->type == NC_MAP) {
                        NcVal *events = nc_index_get(spor, nc_str("events"));
                        if (!events || events->type != NC_LIST) {
                            events = nc_list_new();
                            nc_index_set(spor, nc_str("events"), events);
                        }
                        NcVal *ev = nc_map_new();
                        if (narg>1) nc_index_set(ev, nc_str("name"),   cargs[1]);
                        if (narg>2) nc_index_set(ev, nc_str("status"), cargs[2]);
                        nc_builtin_legg_til(events, ev);
                        fn_r = nc_int(events->list->len);
                    } else { fn_r = nc_int(0); }
                }
                else if (strstr(cn, "trace.felt_tekst") || strstr(cn, "trace.felt_tall")) {
                    /* trace.felt_tekst/felt_tall(spor, key, value) -> spor key count */
                    NcVal *spor = narg>0 ? cargs[0] : nc_nil();
                    if (spor && spor->type == NC_MAP && narg>1 && cargs[1] && cargs[1]->type==NC_STR) {
                        nc_index_set(spor, nc_str(cargs[1]->s), narg>2 ? cargs[2] : nc_nil());
                        fn_r = nc_int(spor->map->len);
                    } else { fn_r = nc_int(0); }
                }
                else if (strstr(cn, "trace.slutt_ok") || strstr(cn, "trace.slutt_feil")) {
                    /* trace.slutt_ok(spor) -> result map */
                    NcVal *spor = narg>0 ? cargs[0] : nc_nil();
                    NcVal *res = nc_map_new();
                    const char *ok_status = strstr(cn, "slutt_ok") ? "ok" : "error";
                    nc_index_set(res, nc_str("status"), nc_str(ok_status));
                    if (spor && spor->type == NC_MAP) {
                        NcVal *events = nc_index_get(spor, nc_str("events"));
                        int ecnt = (events && events->type==NC_LIST) ? events->list->len : 0;
                        char ecbuf[32]; snprintf(ecbuf, sizeof(ecbuf), "%d", ecnt);
                        nc_index_set(res, nc_str("event_count"), nc_str(ecbuf));
                        NcVal *ej = nc_builtin_json_stringify(events ? events : nc_list_new());
                        nc_index_set(res, nc_str("events"), ej ? ej : nc_str("[]"));
                        /* Copy trace metadata for til_logg */
                        NcVal *tid = nc_index_get(spor, nc_str("trace_id"));
                        NcVal *rid = nc_index_get(spor, nc_str("request_id"));
                        NcVal *pth = nc_index_get(spor, nc_str("path"));
                        NcVal *nm  = nc_index_get(spor, nc_str("name"));
                        if (tid) nc_index_set(res, nc_str("trace_id"),   tid);
                        if (rid) nc_index_set(res, nc_str("request_id"), rid);
                        if (pth) nc_index_set(res, nc_str("path"),       pth);
                        if (nm)  nc_index_set(res, nc_str("name"),       nm);
                    } else {
                        nc_index_set(res, nc_str("event_count"), nc_str("0"));
                        nc_index_set(res, nc_str("events"), nc_str("[]"));
                    }
                    fn_r = res;
                }
                else if (strstr(cn, "trace.til_logg")) {
                    /* trace.til_logg(sluttet) -> log map */
                    NcVal *sluttet = narg>0 ? cargs[0] : nc_nil();
                    NcVal *logmap = nc_map_new();
                    if (sluttet && sluttet->type == NC_MAP) {
                        NcVal *tid = nc_index_get(sluttet, nc_str("trace_id"));
                        NcVal *rid = nc_index_get(sluttet, nc_str("request_id"));
                        NcVal *pth = nc_index_get(sluttet, nc_str("path"));
                        NcVal *nm  = nc_index_get(sluttet, nc_str("name"));
                        NcVal *st  = nc_index_get(sluttet, nc_str("status"));
                        nc_index_set(logmap, nc_str("trace_id"),   tid ? tid : nc_str(""));
                        nc_index_set(logmap, nc_str("request_id"), rid ? rid : nc_str(""));
                        nc_index_set(logmap, nc_str("path"),       pth ? pth : nc_str(""));
                        nc_index_set(logmap, nc_str("name"),       nm  ? nm  : nc_str(""));
                        nc_index_set(logmap, nc_str("status"),     st  ? st  : nc_str(""));
                    }
                    fn_r = logmap;
                }
                /* std.log.emit: return JSON string for comparability */
                else if (strstr(cn, "log.emit")) {
                    NcVal *_lv = narg>0 ? cargs[0] : nc_nil();
                    NcVal *_lj = nc_builtin_json_stringify(_lv);
                    fn_r = _lj ? _lj : nc_str("{}");
                }
                 ...
                 * Wrap med setjmp når try_depth > 0 så C-unntak kan fangast av prøv/fang. */
                if (dispatch_first && !local_fn) {
                    if (_had_try) {
                        jmp_buf _disp_saved; memcpy(&_disp_saved,&g_err_jmp,sizeof(jmp_buf));
                        if (setjmp(g_err_jmp)) {
                            memcpy(&g_err_jmp,&_disp_saved,sizeof(jmp_buf));
                            free(cargs); free(callee);
                            if (try_depth>0) {
                                const char *_em=g_err_msg;
                                if(strncmp(_em,"Norscode unntak: ",17)==0)_em+=17;
                                else if(strncmp(_em,"nc-vm feil: Norscode unntak: ",28)==0)_em+=28;
                                strncpy(last_exception,_em,sizeof(last_exception)-1); g_err_msg[0]=0;
                                const char *_cl=try_stack[try_depth-1].catch_lbl;
                                sp=try_stack[try_depth-1].sp_depth;
                                NcVal *_tgt=nc_index_get(label_map,nc_str(_cl));
                                if(_tgt&&_tgt->type==NC_INT){ip=(int)_tgt->i+1;_nc_exn_caught=1;}
                            }
                            fn_r=nc_nil();
                        } else { dispatch_r=nc_dispatch_call(callee,cargs,narg); memcpy(&g_err_jmp,&_disp_saved,sizeof(jmp_buf)); }
                    } else { dispatch_r = nc_dispatch_call(callee, cargs, narg); }
                ...
                /* (_had_try og _nc_exn_caught er allereie deklarert ovanfor) */
                if (_had_try) {
                    jmp_buf _saved_jmp;
...
static NcVal *nc_native_kompiler(const char *src_path, const char *modul) {
    ...
    return nc_builtin_json_stringify(ncb);
}
...
static int nc_try_nc_main_host(void) {
    ...
    if (cmd && !strcmp(cmd, "serve")) {
        /* Serve skal alltid gå vidare til selfhost.nc_main.start utan å bruke run-løypa. */
