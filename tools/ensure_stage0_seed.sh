#!/usr/bin/env bash
# tools/ensure_stage0_seed.sh — sørg for bootstrap/stage0/norscode-<plattform>
#
# Rekkefølge: committed stage0 → GitHub release → (siste utvei) bootstrap/c + clang
set -euo pipefail
...
if [ -f "${ROOT}/bootstrap/c/norscode_generated.c" ] \
    && [ -f "${ROOT}/bootstrap/c/nc_dispatch.c" ]; then
    printf 'ℹ︎ Migrerer seed frå bootstrap/c/ (ein gong — fjern C frå git etterpå)\n' >&2
    NORSCODE_BOOTSTRAP_C=1 bash "$ROOT/tools/build_norscode_native.sh"
    cp "$ROOT/dist/norscode_native" "$DEST"
    chmod +x "$DEST"
    printf '✓ stage0-seed frå bootstrap/c: %s (%d bytes)\n' "$DEST" "$(wc -c < "$DEST" | tr -d ' ')"
    exit 0
fi
